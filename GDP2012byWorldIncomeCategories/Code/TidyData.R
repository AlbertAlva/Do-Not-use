...
GDPSUB$Rank <-as.character(GDPSUB$Rank)
GDPSUB$Economy <-as.character(GDPSUB$Economy)
...
## dataset two
### reading the csv file again for the next chunk of data

GDPSUBRange <-  subset(read.csv("GDPRaw.csv",fill = TRUE,
                         header = FALSE, skip = 221,
                         nrows = 14, blank.lines.skip = TRUE,
                         stringsAsFactors = FALSE,strip.white = TRUE,
                         na.strings = TRUE),select = c(5,6))

names(GDPSUBRange)<- c("IncomeLevel","GDP" )
GDPSUBRange <- GDPSUBRange[c(14,6,4,5,3),]
GDPSUBRange$IncomeLevel<-gsub("income","", GDPSUBRange$IncomeLevel)
GDPSUBRange$GDP <- as.numeric(gsub("[^[:digit:]]","", GDPSUBRange$GDP))
mean(GDPSUBRange$GDP)
str(GDPSUBRange)
head(GDPSUBRange)
...
EDUSUB <- subset(read.csv("eduRaw.csv",fill = TRUE,
                          header = TRUE, blank.lines.skip = TRUE,
                          stringsAsFactors = FALSE,strip.white = TRUE,
                          na.strings = TRUE),select = c(2,3,4))

names(EDUSUB) <- c("CountryCode" , "LongName"  ,  "IncomeGroup")
